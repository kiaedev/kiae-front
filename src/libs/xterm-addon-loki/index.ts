...
export class LokiAddon implements ITerminalAddon {
  ...
  public activate(terminal: Terminal): void {
    this._disposables.push(
      addSocketListener(this._socket, "message", (ev) => {
        ...
        data.streams.forEach((row: any) => {
          const time = dayjs(row.values[0][0] / 1000000).format(
            "MM-DD HH:mm:ss"
          );
          terminal.write(`${time} ${row.stream.container} ${row.values[0][1]}\r\n`);
        });
      })
    );
